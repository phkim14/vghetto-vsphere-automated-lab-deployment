...
$NestedESXiApplianceOVA = 'C:\Users\Administrator\Desktop\vsphere-lab-deploy\Nested_ESXi6.7u3_Appliance_Template_v1.ova'
$VCSAInstallerPath = 'C:\Users\Administrator\Desktop\vsphere-lab-deploy\VMware-VCSA-all-6.7.0-15132721'
...
$ESXiVMNetwork = "VLAN4095" # for Nested ESXi 
$VCSAVMNetwork = "VLAN3116" # for VCSA
$ESXiVLAN = "3116"
$VLAN4095 = "4095"
$VMDatastore = "svcs-5tb"
$VMNetmask = "255.255.255.0"
$VMGateway = "192.168.30.1"
$VMDNS = "192.168.30.31"
$VMNTP = "192.168.30.31"
$VMPassword = "VMware1!"
$VMDomain = "pklab.local"
...
$VAppName = "pklab-vapp-$vSphereVersion-$random_string"
...
if($confirmDeployment -eq 1) {
    ...
    Write-Host -ForegroundColor White $ESXiVMNetwork
...
if($DeploymentTarget -eq "ESXI") {
    ...
    if($VirtualSwitchType -eq "VSS") {
        $network = Get-VirtualPortGroup -Server $viConnection -Name $ESXiVMNetwork
        if($DeployNSX -eq 1) {
            $privateNetwork = Get-VirtualPortGroup -Server $viConnection -Name $PrivateVXLANVMNetwork
        }
    } else {
        $network = Get-VDPortgroup -Server $viConnection -Name $ESXiVMNetwork
        if($DeployNSX -eq 1) {
    ...
    if($DeploymentTarget -eq "VMC") {
        $network = Get-VirtualNetwork -Server $viConnection -Name $ESXiVMNetwork | Select -First 1
    } else {
        if($VirtualSwitchType -eq "VSS") {
            $network = Get-VirtualPortGroup -Server $viConnection -Name $ESXiVMNetwork | Select -First 1
            if($DeployNSX -eq 1) {
                $privateNetwork = Get-VirtualPortGroup -Server $viConnection -Name $PrivateVXLANVMNetwork | Select -First 1
            }
        } else {
            $network = Get-VDPortgroup -Server $viConnection -Name $ESXiVMNetwork | Select -First 1
            if($DeployNSX -eq 1) {
...
if($deployNestedESXiVMs -eq 1) {
    if($DeploymentTarget -eq "ESXI") {
        ...
        $NestedESXiHostnameToIPs.GetEnumerator() | Sort-Object -Property Value | Foreach-Object {
            ...
            $ovfconfig.NetworkMapping.$networkMapLabel.value = $ESXiVMNetwork
            ...
            My-Logger "Adding 2 more network adapters ..."
            New-NetworkAdapter -VM $vm -NetworkName $ESXiVMNetwork -StartConnected
            New-NetworkAdapter -VM $vm -NetworkName $ESXiVMNetwork -StartConnected
...
if($DeployNSX -eq 1) {
    if($DeploymentTarget -eq "VCENTER" -or $DeploymentTarget -eq "VMC") {
        ...
        $ovfconfig.NetworkMapping.VSMgmt.value = $VCSAVMNetwork
...
if($deployVCSA -eq 1) {
    if($DeploymentTarget -eq "ESXI") {
        ...
        $config.new_vcsa.esxi.deployment_network = $VCSAVMNetwork
        ...
        $config.new_vcsa.vc.deployment_network = $VCSAVMNetwork
...
if($setupNewVC -eq 1) {
    ...
    if($configureVDS -eq 1) {
        My-Logger "Creating Management VDS ..."
        New-VDSwitch -Name $MgmtVDSName -Location $NewVCDatacenterName -NumUplinkPorts 2 -Mtu $MTU
        ...
        foreach ($vmhost in Get-Cluster -Server $vc | Get-VMHost) {
            # add hosts to Management and Edge VDS
            Get-VDSwitch -Name $MgmtVDSName | Add-VDSwitchVMHost -VMHost $vmhost
            Get-VDSwitch -Name $EdgeVDSName | Add-VDSwitchVMHost -VMHost $vmhost

            # add vmnic1 to Management VDS
            ...
            # Move vmnic0 to Edge VDS
            $vmhostNetworkAdapter1 = Get-VMHost $vmhost | Get-VMHostNetworkAdapter -Physical -Name vmnic0
            Get-VDSwitch -Name $EdgeVDSName | Add-VDSwitchPhysicalNetworkAdapter -VMHostPhysicalNic $vmhostNetworkAdapter1 -Confirm:$false | Out-Null

            # Create vMotion and VSAN vmks
            $vmk0 = Get-VMHostNetworkAdapter -Server $vc -Name vmk0 -VMHost $vmhost
